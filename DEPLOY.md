...
| Issue | Fix |
|-------|-----|
| "This endpoint is no longer available. Please upgrade to v3 APIs" | Redeploy latest code (uses Composio v3). Ensure `LINKEDIN_TOOLKIT_VERSION=20260424_00` |
| Mock message in response | Set `MOCK_LINKEDIN_PUBLISH=false` and redeploy |
| Not connected | Reconnect LinkedIn in Composio |
| Author URN error | Run `node scripts/check-linkedin-setup.js` and set `LINKEDIN_AUTHOR_URN` in Render |
| 426 Upgrade Required | Set `LINKEDIN_TOOLKIT_VERSION=20260424_00` (or latest from Composio dashboard) |
